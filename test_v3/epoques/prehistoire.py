# -*- coding: utf-8 -*-
# epoques/prehistoire.py — ÈRE PRÉHISTORIQUE (flammes supprimées)
import pygame
import math
...
class PrehistoireRoom(BaseRoom):
    """Salle préhistorique. Flammes désactivées — draw_epoch_decoration vide."""
    def __init__(self, game):
        super().__init__(game, "prehistoire")
        self.epoch_name_display = "ÈRE PRÉHISTORIQUE"
        self.theme_color        = PREHISTOIRE_COLOR

    def on_enter(self):  print("Bienvenue à l'âge de pierre !")
    def on_exit(self):   print("L'homme préhistorique évolue !")

    def draw_epoch_decoration(self, surface):
        """
        Aucune décoration active.
        Pour remettre les flammes, décommenter ce qui suit :
