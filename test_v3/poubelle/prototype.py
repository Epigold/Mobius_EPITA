...
def draw_main_menu():
    ...
    title = font_large.render("MOBIUS ROGUELIKE", True, WHITE)
    screen.blit(title, title.get_rect(center=(screen_width//2, 150)))

    global play_button, settings_button, quit_button

    play_button = pygame.Rect(screen_width//2 - 150, 300, 300, 60)
    quit_button = pygame.Rect(screen_width//2 - 150, 460, 300, 60)

    mouse_pos = pygame.mouse.get_pos()

    def draw_button(rect, text):
        color = (80, 80, 200) if rect.collidepoint(mouse_pos) else (50, 50, 150)
        pygame.draw.rect(screen, color, rect, border_radius=10)
        txt = font_medium.render(text, True, WHITE)
        screen.blit(txt, txt.get_rect(center=rect.center))

    draw_button(play_button, "Nouvelle Partie")
    draw_button(quit_button, "Quitter")
...
def draw_volume_slider():
    ...
    if mouse_pressed[0]:
        if slider_x <= mouse_pos[0] <= slider_x + slider_width and \
           slider_y - 10 <= mouse_pos[1] <= slider_y + 20:
    ...
    pygame.draw.rect(screen, (80, 160, 255),
                     (slider_x, slider_y, volume * slider_width, slider_height),
                     border_radius=4)
    ...
    # Texte
    vol_text = font_small.render("Volume", True, WHITE)
    screen.blit(vol_text, (slider_x, slider_y - 20))
#######################################################################################################################################################################################################################
